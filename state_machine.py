...
class StateMachine:
    ...
    def update(self, frame_hash, score, error):
        """
        ...
        if error:
            # Preserve internal self.state so it doesn't flip to MISSING on transient errors
            err_state = self._error_to_state(error)
            self.consecutive_failures += 1
            # We return the specific error state for the UI/Storage
            return err_state, display_score
